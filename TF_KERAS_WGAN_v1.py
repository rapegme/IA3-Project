# -*- coding: utf-8 -*-
"""
Created on Mon Mar  1 12:45:32 2021
...
#Process the images before training the model
def preprocess(fn):
...
optD = tf.keras.optimizers.Adam(lr=0.0001, beta_1=0.5, beta_2=0.9)
optG = tf.keras.optimizers.Adam(lr=0.0001, beta_1=0.5, beta_2=0.9)
...
def save_images(model, epoch, test_input):
    ...
    plt.savefig('./WGAN/Img/image_at_epoch_{:04d}.png'.format(epoch))

def save_weights (discriminator, generator):
    
    discriminator.save_weights("./WGAN/D/pesos_D")
    generator.save_weights("./WGAN/G/pesos_G")
...
def train(dataset, epochs, D_count, G_count):
